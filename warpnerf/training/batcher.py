...
def init_training_rays_and_pixels_kernel(
    ...
    rand_idx: wp.array1d(dtype=wp.uint64),
    ...
    img_w = image_dims[0]
    img_h = image_dims[1]
    n_pixels_per_image = wp.uint64(img_w * img_h)
    ...
    image_idx = wp.int32(rand_idx[i] // n_pixels_per_image)

    pixel_x = wp.int32(rand_pixel_idx % wp.uint64(img_w))
    pixel_y = wp.int32(rand_pixel_idx // wp.uint64(img_w))
